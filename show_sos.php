...
/*$soi_constituents = "SELECT DISTINCT soi.description as soi_description, cs.name as cs_name, op.description as op_description, r.description as mission_requirement, m.name as cs_mission FROM constituent cs, mission m, state_of_interest soi,requirement r, observation_point op, transient_configuration tac, composition c WHERE 
	soi.id_requirement = r.id_requirement AND r.id_mission = m.id_mission AND m.id_constituent = cs.id_constituent AND soi.id_sos = $id_sos AND op.id_sos = $id_sos AND op.id_soi = soi.id_soi AND tac.id_sos = $id_sos AND tac.id_transient_configuration = c.id_transient_configuration";*/
#SELECT * FROM state_of_interest WHERE id_sos = $id_sos
/*
SELECT DISTINCT soi.id_soi, soi.description as soi_description, cs.name as cs_name, op.description as op_description, r.description as mission_requirement, m.name as cs_mission FROM constituent cs, mission m, state_of_interest soi,requirement r, observation_point op, transient_configuration tac, composition c WHERE soi.id_requirement = r.id_requirement AND r.id_mission = m.id_mission AND m.id_constituent = cs.id_constituent AND soi.id_sos = 17 and op.id_sos = 17 AND op.id_soi = soi.id_soi AND tac.id_sos = 17 AND tac.id_transient_configuration = c.id_transient_configuration
*/
